...
import { User, FileText, Info, CheckCircle2 } from "lucide-react";
...
const PatientProfileForm = () => {
  ...
  const [form, setForm] = useState(() => {
    ...
    const regId = `PAT-${Date.now()}-${Math.random().toString(36).substring(2, 8).toUpperCase()}`;
      ...
      <div className="w-full max-w-3xl">
        ...
        <div className="flex items-center justify-center mb-6">
          <div className="flex gap-4">
            ...
            <div className={`h-1 w-8 rounded bg-gradient-to-r from-blue-400 to-gray-400 ${step >= 2 ? '' : 'opacity-30'}`}></div>
            <div className={`flex flex-col items-center ${step >= 2 ? 'text-gray-700' : 'text-gray-400'}`}> <FileText size={28} /><span className="text-xs mt-1">Legal</span></div>
          </div>
        </div>
        <Card className="shadow-2xl rounded-3xl border-0">
          <CardHeader className="bg-gradient-to-r from-blue-800 to-green-600 rounded-t-3xl p-10 mb-6 shadow-xl flex flex-col items-center animate-fade-in">
            ...
            <div className="flex items-center gap-4">
              <CardTitle className="text-white text-3xl font-extrabold tracking-wide drop-shadow animate-slide-in">Patient Profile Information</CardTitle>
            </div>
            <p className="text-blue-100 mt-3 text-base font-medium text-center">Please fill out all required fields to complete your patient profile. All information is kept confidential.</p>
          </CardHeader>
